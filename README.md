...
**How "already checked" is tracked:** when a book has no audiobook, the script writes the sentinel value `'none'` to `audiobook_audible_url` (instead of leaving it `null`). On every run the script filters on `audiobook_audible_url IS NULL` — so only books that were never checked are processed. Confirmed audiobooks get a real Audible search URL. This means the script is always safe to re-run; it only processes new books.

> Note: `audiobook_available` defaults to `false` in the DB for all rows, so filtering on that column would never find new/unchecked books. Always use `audiobook_audible_url IS NULL` as the "needs checking" signal.

```bash
node scripts/fill-audiobooks.mjs               # only never-checked (audible_url IS NULL) — safe to re-run
...
Pagination is built in (`.range()`) so the script handles databases larger than the Supabase 1000-row default cap.
...
See [docs/todo.md](docs/todo.md) for future plans: Reddit bot, hybrid static/server architecture, and affiliate links setup.
