...
const GROUPS = [
  {
    ...
    items: [
      ...
      { label: 'Detect series (full pipeline)', cmd: 'detect-series.mjs',
        desc: 'Full pipeline: Regex → Open Library (no quota) → Google Books → Hardcover → Gemini 2.5 Pro. High-confidence auto-apply; lower goes to /admin/series-review. Run after ISBN backfill.' },
      { label: 'Detect series (Open Library only)', cmd: 'detect-series.mjs', args: '--pass-ol',
        desc: 'Free, no-quota pass using Open Library ISBN lookup. Fast first step — handles most well-known series. Run before the LLM pass.' },
      { label: 'Detect series (LLM only — 2.5 Pro)', cmd: 'detect-series.mjs', args: '--pass-3',
        desc: 'Gemini 2.5 Pro pass on books that still have no series after prior passes. Groups by author, accuracy-first prompt. Uses Vertex AI credit.' },
      { label: 'Detect series (confirm pending — 2.5 Pro)', cmd: 'detect-series.mjs', args: '--confirm-pending',
        desc: 'Re-evaluates 2,290 pending-review series assignments with Gemini 2.5 Pro. Confirms obvious ones automatically, rejects wrong ones, leaves genuinely ambiguous cases for manual review. Run once to shrink the review queue.' },
