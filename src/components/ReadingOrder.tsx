  ...
  const inner = (
    <div className="flex flex-col items-center text-center group w-24 sm:w-32 shrink-0">
      ...
      <div className="relative w-full flex justify-center mb-2">
        ...
        <div
          className={`relative z-10 w-7 h-7 rounded-full ${cfg.dot} flex items-center justify-center text-white font-bold text-xs shadow shrink-0`}
        >
          {index + 1}
        </div>
      </div>
      ...
      {/* Title */}
      <p
        className={`text-xs font-semibold text-zinc-900 leading-snug line-clamp-2 mb-0.5 ${book.slug ? 'group-hover:text-purple-700 transition-colors' : ''}`}
      >
        {book.title}
      </p>
      ...
      {book.position != null && (
        <p className="text-[10px] text-purple-400 font-medium leading-snug mb-0.5">
      ...
      {/* Status badge */}
      <span
        className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border ${cfg.pill}`}
      >
        {cfg.label}
      </span>
      ...
      {book.darkness_level != null && book.darkness_level >= 1 && (
        <div
          ...
          title={`Darkness: ${book.darkness_level}/5`}
        >
          ...
          <div className="flex items-center gap-0.5">
            {[1, 2, 3, 4, 5].map((i) => (
              <span
                key={i}
                className={`w-1.5 h-1.5 rounded-full ${i <= book.darkness_level! ? 'bg-zinc-600' : 'bg-zinc-200'}`}
              />
            ))}
          </div>
        </div>
...
export default function ReadingOrder({
  ...
  const sourceGroups: BookGroup[] =
    groups ?? (books ? [{ label: '', books }] : []);

  const groupsWithIndex = sourceGroups.map((g, gi) => ({
    ...g,
    startIndex: sourceGroups
      .slice(0, gi)
      .reduce((sum, prev) => sum + prev.books.length, 0),
  }));
        ...
        {groupsWithIndex.map((group) => (
          <div key={group.label || 'group'}>
            ...
            {group.note && group.noteType && (
              <div
                className={`mb-4 rounded-lg border ${noteStyle[group.noteType].border} ${noteStyle[group.noteType].bg} px-4 py-3`}
              >
                <p className={`text-sm ${noteStyle[group.noteType].text}`}>
                  <span className="mr-1.5">
                    {noteStyle[group.noteType].emoji}
                  </span>
                  {group.note}
                </p>
              </div>
        ...
        <div className="mt-8 flex flex-wrap gap-5 text-sm border-t pt-6">
          {(
            Object.entries(statusConfig) as [
              string,
              (typeof statusConfig)['mandatory'],
            ][]
          ).map(([key, cfg]) => (
            <div key={key} className="flex items-center gap-2">
              <span
                className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold border ${cfg.pill}`}
              >
                {cfg.label}
              </span>
