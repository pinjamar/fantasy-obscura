...
export const GET: APIRoute = async ({ url }) => {
  ...
  try {
    ...
    if (source === 'bigbook') {
      const baseUrl = import.meta.env.BIGBOOK_API_BASE || '';
      const apiKey = import.meta.env.BIGBOOK_API_KEY;
      const searchPath = import.meta.env.BIGBOOK_API_SEARCH_PATH || '/search';
      const items = await fetchBigbook(q, baseUrl, apiKey, searchPath);
      return new Response(JSON.stringify({ items }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      });
